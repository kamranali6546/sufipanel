                ...
                if(!empty($data['reportData']))
                {
                    foreach($data['reportData'] as $obj)
                    {
                        ...
                        $fileNumber=idToName('company','id',$obj->company,'company_Code')."-".$obj->id;
                           ...
                           $this->excel->getActiveSheet()->setCellValue('L'.$j,round($payableSupplier,1));
                           $this->excel->getActiveSheet()->setCellValue('M'.$j,round($additionalCharges,2));
                           $this->excel->getActiveSheet()->setCellValue('N'.$j,round($total_ticket_cost,1));
                           $this->excel->getActiveSheet()->setCellValue('O'.$j,round($profit_gross));
                    ...
                    $this->excel->getActiveSheet()->setCellValue('K'.$lastLine,round($totalPriceSale,1));
                    $this->excel->getActiveSheet()->setCellValue('L'.$lastLine,round($totalPayableSullpier,1));
                    $this->excel->getActiveSheet()->setCellValue('M'.$lastLine,round($totalAdditionalCharges));
                    $this->excel->getActiveSheet()->setCellValue('N'.$lastLine,round($toCost_grand,1));
                    $this->excel->getActiveSheet()->setCellValue('O'.$lastLine,round($issuedTotal,1));                 
...
//                $this->excel->getActiveSheet()->getStyle('A'.$canTi)->applyFromArray(array('font' => array('size' => 17,'bold' => true,'color' => array('rgb' => '0b6623'))));  
                ...
                if(!empty($data['reportDataCancelData']))
                {
                    ...
                    $this->excel->getActiveSheet()->setCellValue('M'.$CancelLastLine,round($totalAdditionalCharges_cancel));
                    $this->excel->getActiveSheet()->setCellValue('N'.$CancelLastLine,round($toCost_grand_cancel));                 
                    $this->excel->getActiveSheet()->setCellValue('O'.$CancelLastLine,round($cancelTotal,1));                 
                ...
                $this->excel->getActiveSheet()->setCellValue('O'.$grandline,round(($issuedTotal+$cancelTotal),1));
                    ...
                    foreach ($issedData as $row)
                        {
                           ...
                           $total_segments=$total_segments+$segmentCoun;
                           $receiveCount=round(totalreceivedAmount($row->id),2);
                           $customersReceives=$customersReceives+$receiveCount;
                           $payableSupplier=round(ticketCost($row->id),2);
                           $totalPayableSullpier=$totalPayableSullpier+$payableSupplier;
                           $additionalCharges=(round(ticketCharges($row->id),2)+round(ticketChargesAditional($row->id),2));
                           $totalAdditionalCharges=$totalAdditionalCharges+$additionalCharges;
                           $total_ticket_cost=$additionalCharges+$payableSupplier;
                           $toCost_grand=$toCost_grand+$total_ticket_cost;
                           $profit_gross=round($receiveCount,2)-round($total_ticket_cost,2);
                           $issuedTotal=$issuedTotal+$profit_gross;
...
//                $this->excel->getActiveSheet()->getStyle('A'.$canTi)->applyFromArray(array('font' => array('size' => 17,'bold' => true,'color' => array('rgb' => '0b6623'))));  
            ...
            'color' => array('argb' => 'FFFF0000'),
               ...
               // $this->excel->getActiveSheet()->mergeCells('M'.$expeneLineWrite.':O'.$expeneLineWrite);
                $this->excel->getActiveSheet()->setCellValue('O'.$expeneLineWrite,'-'.$expenseTotal);
