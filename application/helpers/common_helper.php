...
function adjustmentPay($bookingId)
{
     $ci =& get_instance();
     $ci->load->database();
     $qry=" Select sum(amount) as total from payment where booking_ref=$bookingId AND pay_type='Cr' And payment_nature='supplier' AND pay_by!='Customer' AND pay_by!='Air Ticket Purchases'  ";
     $query = $ci->db->query($qry);
...
function paymentReceived($bookingID)
{
    return (sumOfAmount('payment','amount',array('pay_to'=>'Bank Sufi HSBC','pay_type'=>'Dr','booking_ref'=>$bookingID))+ sumOfAmount('payment','amount',array('pay_to'=>'Bank Sufi Arro Money','pay_type'=>'Dr','booking_ref'=>$bookingID))+ sumOfAmount('payment','amount',array('pay_to'=>'Ali Personal Barclays','pay_type'=>'Dr','booking_ref'=>$bookingID))+ sumOfAmount('payment','amount',array('pay_to'=>'SUFI REVOLT','pay_type'=>'Dr','booking_ref'=>$bookingID)) + sumOfAmount('payment','amount',array('pay_to'=>'SUFI TRANSFERWISE','pay_type'=>'Dr','booking_ref'=>$bookingID))  + sumOfAmount('payment','amount',array('pay_to'=>'PAYPALL Sufi','pay_type'=>'Dr','booking_ref'=>$bookingID)) + sumOfAmount('payment','amount',array('pay_to'=>'SquareUp Sufi Travels','pay_type'=>'Dr','booking_ref'=>$bookingID)) + sumOfAmount('payment','amount',array('pay_to'=>'ST Cooperative Bank','pay_type'=>'Dr','booking_ref'=>$bookingID)) +  sumOfAmount('payment','amount',array('pay_to'=>'Travel Pack','pay_type'=>'Dr','booking_ref'=>$bookingID)) + sumOfAmount('payment','amount',array('pay_to'=>'TSB BANK SUFI','pay_type'=>'Dr','booking_ref'=>$bookingID)) );
}
function cardPaymentSum($bookingID)
{
    return (sumOfAmount('payment','amount',array('pay_to'=>'Card Terminal-BH Squareup','payment_nature'=>'bank','pay_type'=>'Dr','booking_ref'=>$bookingID)) + sumOfAmount('payment','amount',array('pay_to'=>'Square Up card terminal SUFITRAVELS','pay_type'=>'Dr','booking_ref'=>$bookingID)) + sumOfAmount('payment','amount',array('pay_to'=>'Card Charge Bright Holiday','pay_type'=>'Dr','booking_ref'=>$bookingID)) + sumOfAmount('payment','amount',array('pay_to'=>'CARD TERMINAL PAYPALL','pay_type'=>'Dr','booking_ref'=>$bookingID)) + sumOfAmount('payment','amount',array('pay_to'=>'STRIPE CARD CHARGE','pay_type'=>'Dr','booking_ref'=>$bookingID)) );
}
...
function bankPayment($bookingId)
{
    ...
    return paymentReceived($bookingId);
...
function supplierPayAmount($bookingId)
{
    $ci =& get_instance();
    $ci->load->database();
    $ary="Select sum(amount) as amount from payment where booking_ref='$bookingId' AND payment_nature='supplier' AND pay_type='Dr' AND pay_to!='Credit Card Charge Global Travel' AND pay_to!='Debit Card Charge Global Travel' And pay_to!='Card Charge Bright Holiday' AND pay_by!='Air Ticket Sales-UK' AND pay_by!='Expense-Air Ticket Purchases' ";
     $response=$ci->db->query($ary);
    if($response->num_rows()>0)
    {
        $rowRes= $response->row_array();
...
function progressSheetCount($agentId=0,$brand=0)
{
    $ci =& get_instance();
    $ci->load->database();
    $datae=date('Y-m-d');
    ...
    {
       $qry=" Select id from booking_details where booking_date BETWEEN '$startDate' AND '$datae' AND booking_under_brand='$brand' ";   
    }
    else
    {
...
function countLoginArea($brand='')
{
    $ci =& get_instance();
    $ci->load->database();
    if(!empty($brand))
    {
       $qry=' select id  from admin  where first_name!="special" And company="'.$brand.'" '; 
    }
    else
    {
...
function generalReportCount($id='',$brand=0)
{
    $ci =& get_instance();
    $ci->load->database();
    $startDate=date('Y-m-01');
    ...
    else if($brand >0)
    {
        $condition.="  a.issue_date BETWEEN '$startDate'  AND  '$endDate' AND " ;
        $condition2.="  a.cancel_date BETWEEN '$startDate'  AND  '$endDate' AND " ; 
         $condition.="  a.booking_under_brand='$brand'  ";
         $condition2.="  a.booking_under_brand='$brand'  ";
    }
    else
    {
       $condition.="  a.issue_date BETWEEN '$startDate'  AND  '$endDate' AND " ;
...
//            $receiveCount=sumOfAmount('payment','amount',array('pay_to'=>'SUFI REVOLT','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id']))+ sumOfAmount('payment','amount',array('pay_to'=>'Bank Sufi HSBC','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id']))+ sumOfAmount('payment','amount',array('pay_to'=>'Bank Sufi Arro Money','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id']))+ sumOfAmount('payment','amount',array('pay_to'=>'Ali Personal Barclays','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id'])) + sumOfAmount('payment','amount',array('pay_to'=>'Credit Card Charge Global Travel','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id']))+ sumOfAmount('payment','amount',array('pay_to'=>'Debit Card Charge Global Travel','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id'])) + sumOfAmount('payment','amount',array('pay_to'=>'SUFI TRANSFERWISE','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id'])) + sumOfAmount('payment','amount',array('pay_to'=>'Square Up card terminal SUFITRAVELS','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id'])) +sumOfAmount('payment','amount',array('pay_to'=>'Card Charges Square Up card terminal','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id'])) + sumOfAmount('payment','amount',array('pay_to'=>'Card Charge Bright Holiday','pay_type'=>'Dr','booking_ref'=>$objissuedObj['id']));
            $receiveCount=( paymentReceived($objissuedObj['id']) + cardPaymentSum($objissuedObj['id']));
