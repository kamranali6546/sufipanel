 ...
 */
 public function __construct()
    ...
    public function index()
    {
        ...
        $bankFliterName=  $this->input->post('bankName');
        ...
        {
            $localbank=$this->BaseModel->getWhereM('bank',array('flag'=>1,'bank_type'=>'Pk','bank_name'=>$bankFliterName));
        }
        else
        {
       ...
       // $localbank=$this->BaseModel->getWhereM('bank',array('flag'=>1,'bank_type'=>'Pk'));
        ...
        if(!empty($localbank))
        {
            ...
            {
                $localBankArray[]="$loBank->bank_name";
                if($key==0)
                {
                    $listdata.="'$loBank->bank_name'";
                }
                else
                {
                    $listdata.=",'$loBank->bank_name'";
       ...
       // $listdata=  implode(',', $localBankArray);
        ...
        {
             $qry=" Select * from payment where pay_to IN (".$listdata.") AND payment_nature='bank' AND  pay_date BETWEEN '$opingDate' AND '$closingDate' order by pay_date ASC ";
             $closeQry="Select * from payment where pay_to IN (".$listdata.") AND payment_nature='bank' AND pay_date <='$closingBalanceEndDate'  order by pay_date ASC ";
        }
        else
        {
            $qry=" Select * from payment where pay_to IN (".$listdata.") AND payment_nature='bank' AND  pay_date BETWEEN '$opingDate' AND '$closingDate' order by pay_date ASC ";
