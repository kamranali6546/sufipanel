    ...
    public function index()
    {
       ...
       // $data['record']=$this->BaseModel->gettingJoin('admin','company','left','admin.company','company.id');
        if($this->session->userdata('flag')==3)
        {
            ...
            $data['record']=$this->BaseModel->getQuery('select  admin.id as adminId,admin.first_name,admin.last_name,admin.cnic,admin.email,admin.cell,admin.login_name,admin.password,admin.pic,admin.flag,admin.profitTarget,admin.sales_target,admin.agent_status,company.id as companyId, company.company_name from admin LEFT JOIN company  ON  admin.company=company.id where first_name!="special" And admin.company='.$company);
        }
        else if($this->session->userdata('flag')==1)
        {
...
//              {
                       ...
                       $sales_target=$this->input->post('salesTarget');
                       $profit_target=$this->input->post('profitTarget');
                       $adminL=  $this->input->post('adminLevelsFlag');
                       if($adminL==1)
                        {
//                           exit();
                            $dataArray=array(
                        ...
                        else
                        {
                            ...
                            if($company==2)
                            {
                                $newariavlebrandcode="SC";
                            }
                            if($company==3)
                            {
                                $newariavlebrandcode="QF";
                            }
                             $dataArray=array(
                           'first_name'=>$first_name,
                           'last_name'=>$last_name,
                           ...
                           'password'=>$password,
                           'company'=>$company,
                          'brand_code'=>$newariavlebrandcode,
                           'shift_time_start'=>$shift_time_start,
                           'shift_time_end'=>$shift_time_end,
                           'sales_target'=>$sales_target,
                           'profitTarget'=>$profit_target,
