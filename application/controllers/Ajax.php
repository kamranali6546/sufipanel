...
//        $res=$this->BaseModel->del('inquiry',array('id'=>$id));
        $res=$this->BaseModel->update('inquiry',array('status'=>3),array('id'=>$id));
        if($res >0)
        {
           echo $res;
        }
    ...
    public function deteleInquiry2()
    {
        $id=$this->input->post('ids');
        $this->BaseModel->del('agent_comments',array('inquiry_id'=>$id));
        $res=$this->BaseModel->del('inquiry',array('id'=>$id));
...
//        $this->BaseModel->del('agent_comments',array('inquiry_id'=>$id));
//        $res=$this->BaseModel->del('inquiry',array('id'=>$id));
        $res=$this->BaseModel->update('inquiry',array('status'=>1),array('id'=>$id));
        if($res >0)
        {
           echo $res;
        }
...
//            }
//        }
        if($supplierReference!='' && preg_match('/[a-zA-Z0-9]+/', $supplierReference))
        {
...
//       $remarks=$this->input->post('agentremarks'); 
    ...
    public function tOrderSend()
    {
       ...
       $this->BaseModel->save('event_history',array('booking_id'=>$bookingOfTickerOrder,'agent_id'=>$agentId,'event_type'=>'Auto','event'=>$logData,'flag'=>1));
        $adminMailAddress='admin@sufitravelandtours.co.uk';
            $adminSubject='';
            $agentSubject='';
            $adminConfig=Array(       
                    'mailtype' => 'html',
            ...
            //$companyName='Bright HoliDay';
             $adminSubject='Ticket Order Request For File Number '.$companyCode.'-'.$bookingOfTickerOrder;
                ...
                $agentData['messageData']='Your ticket order request has been sent for the File no '.$companyCode.'-'.$bookingOfTickerOrder.'  with  PNR  '.$ticketOrderPnr.'  and ISSUED COST '.$ticketOrderIssueCost.' <br><br>In case of any error regarding ticket Order please inform to your admin department.<br><br>
                        This message was sent from a notification email address that does not accept incoming email. Please do not reply to this message.
                        If you have any question regarding your concern, please feel free to email us on admin through admin@sufitravelandtours.co.uk
                        ';
                $this->load->library('email', $adminConfig);
                $this->email->set_newline("\r\n");
                $this->email->from($from,$companyName);
...
//                $this->email->send();
    ...
    public function doSendReceipt()
    {
        ...
        $zohoPassword=idToName('admin','email',$from,'zoho_password');
        $file=$_FILES['attachment'];
        $EmailConfig=Array(
            'protocol'=>'smtp',
...
//            'charset'   => 'iso-8859-1'
         ...
         $mailData=array(
            ...
            $mailData['messageData']= str_replace("highlighttext", "black",$this->input->post('receiptSendMessage'));
//            print_r($this->input->post('receiptSendMessage'));
//            exit();
...
//            'protocol'=>'smtp',
//            'smtp_host'=>'smtp.zoho.com',
//            'smtp_user' => 'admin@sufitravelandtours.co.uk',
//            'smtp_pass' => 'vl[lkp2a6TVH',
//            'smtp_port' => 587,
//           'smtp_crypto' => "tls",
//            'mailtype' => 'html',
        ...
        if($res)
            {
                ...
                $qru=" Select sum(amount) as total from  payment where pay_to='Customer' AND pay_type='Dr' AND booking_ref='$bookingId' AND description!='Tickets Supplied To Customer' ";
                $amountGotoCustomer=getQueryRes($qru);
//                $amountGotoCustomer=sumOfAmount('payment','amount',array('pay_to'=>'Customer','pay_type'=>'Dr','booking_ref'=>$bookingId,'description!'=>'Air Ticket Purchases'));
                $refendAmount=(refundSum($bookingId)+$amountGotoCustomer);
                $customerPayed=paymentReceived($bookingId) + cardPaymentSum($bookingId);
                $netPay=round($customerPayed-$refendAmount,2);
                if($netPay>0)
                {
                    $this->BaseModel->save('payment',array('transectionId'=>$trId,'pay_to'=>'Profit & Loss on Cancellations-UK','pay_by'=>'Customer','amount'=>$netPay,'booking_ref'=>$bookingId,'pay_date'=>date('Y-m-d'),'pay_type'=>'Cr','description'=>'Profit on cancelled file','payment_nature'=>'income','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
                    $this->BaseModel->save('payment',array('transectionId'=>$trId,'pay_to'=>'Customer','pay_by'=>'Profit & Loss on Cancellations-UK','amount'=>$netPay,'booking_ref'=>$bookingId,'pay_date'=>date('Y-m-d'),'pay_type'=>'Dr','description'=>'Profit on cancelled file','payment_nature'=>'supplier','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
                }
    ...
    public function doIssue()
    {
        ...
        $getLogIss=  $this->BaseModel->getWhereM('event_history',array('booking_id'=>$bookingId,'event'=>'Tickets Issued.'));
        if(!empty($getLogIss))
        {
            if(!empty($getLogIss[0]))
            {
...
//                $this->email->send();
    ...
    public function returnToPending()
    {
        ...
        $this->BaseModel->del('payment',array('booking_ref'=>$bookingId,'pay_to'=>'Profit & Loss on Cancellations-UK'));
        $this->BaseModel->del('payment',array('booking_ref'=>$bookingId,'pay_by'=>'Profit & Loss on Cancellations-UK'));
    ...
    public function returnFromIssued()
    {
        ...
        $this->BaseModel->del('payment',array('booking_ref'=>$bookingId,'pay_to'=>'Expense-Air Ticket Purchases'));
        $this->BaseModel->del('payment',array('booking_ref'=>$bookingId,'pay_by'=>'Expense-Air Ticket Purchases'));
        
        $this->BaseModel->del('payment',array('booking_ref'=>$bookingId,'pay_by'=>'Air Ticket Sales-UK'));
        $this->BaseModel->del('payment',array('booking_ref'=>$bookingId,'pay_to'=>'Air Ticket Sales-UK'));
    ...
    public function markedClear()
    {
        ...
        $this->BaseModel->save('payment',array('transectionId'=>$trId,'pay_to'=>'Expense-Air Ticket Purchases','pay_by'=>$supplier,'amount'=>$payableTosupplier,'booking_ref'=>$bookingId,'pay_date'=>date('Y-m-d'),'pay_type'=>'Dr','description'=>'Tickets Purchased From Supplier','payment_nature'=>'expense','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
        $this->BaseModel->save('payment',array('transectionId'=>$trId,'pay_to'=>$supplier,'pay_by'=>'Expense-Air Ticket Purchases','amount'=>$payableTosupplier,'booking_ref'=>$bookingId,'pay_date'=>date('Y-m-d'),'pay_type'=>'Cr','description'=>'Tickets Purchased From Supplier','payment_nature'=>'supplier','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
         
        
        $this->BaseModel->save('payment',array('transectionId'=>$trId2,'pay_to'=>'Customer','pay_by'=>'Air Ticket Sales-UK','amount'=>$customerPayed,'booking_ref'=>$bookingId,'pay_date'=>date('Y-m-d'),'pay_type'=>'Dr','description'=>'Tickets Supplied To Customer','payment_nature'=>'supplier','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
        $this->BaseModel->save('payment',array('transectionId'=>$trId2,'pay_to'=>'Air Ticket Sales-UK','pay_by'=>'Customer','amount'=>$customerPayed,'booking_ref'=>$bookingId,'pay_date'=>date('Y-m-d'),'pay_type'=>'Cr','description'=>'Tickets Supplied To Customer','payment_nature'=>'income','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
    ...
    public function getViewInquiryDetails()
    {
        ...
        $obj=$record[0];
//        echo date('d-M-y',strtotime($obj->departure_date));
         $ite=$obj->flight_itenery;
      $itenery=unserialize(base64_decode($ite));
//        $itenery= json_decode($obj->flight_itenery,true); 
//         echo  $itenery['FareInfo'];
//        echo "<pre>";
//        print_r($itenery);
//        echo "</pre>";
        $dataArray['itenery']=$ite;
        $itenerData=$this->load->view('itenery_details_view.php',$dataArray,true);
        $style="<style>.list li {    text-decoration: none;list-style: none;}</style>";
        $html=$style.'<table class="table" width="100%">
                        ...
                        <tr>
                                ...
                                <td>'. $obj->infaunt.'</td>
                                <td><b>Inquiry Source</b></td>
                                <td>'. originInquiry($obj->inquiry_regin).' </td>
                        </tr>
		</table><br>
                <br>
    ...
    <h4 class="text-underline" style="color: #000000">Outbond Flight Details</h4>
    <div class="row ml-15">
        <div class="col-md-8">
            <h5 class="list-title" style="color: #000000">LHR-HEATHROW to BRU-BRUSSELS</h5>
            <ul class="list" style="color:#000000;">
                <li>SN BRUSSELS AIRLINES &nbsp; </li>
                <li>SN2104</li>
                <li>Depart 06:50 Arrive 08:55</li>
                <li>Duration:01H 05m</li>
            </ul>
            <h5 style="color: red">Stopover:02H 15m </h5>
        </div>
    </div>
    <div class="row ml-15">
        <div class="col-md-8">
            <h5 class="list-title" style="color: #000000">BRU-BRUSSELS to ACC-ACCRA</h5>
            <ul class="list" style="color:#000000;">
                <li>SN BRUSSELS AIRLINES &nbsp; </li>
                <li>SN277</li>
                <li>Depart 11:10 Arrive 16:55</li>
                <li>Duration:09H 00m</li>
            </ul>
        </div>
    </div>
    <hr style="height: 3px !important;color:red;">
    <h4 class="text-underline" style="color:#000000">Incoming Flight Details</h4>
    <div class="row ml-15">
        <div class="col-md-8">
            <h5 class="list-title" style="color:#000000">ACC-ACCRA to BRU-BRUSSELS</h5>
            <ul class="list" style="color:#000000">
                <li>SN BRUSSELS AIRLINES&nbsp;</li>
                <li>SN278</li>
                <li>Depart 23:25 Arrive 07:05</li>
                <li>Duration:06H 40m</li>
            </ul>
            <h5 style="color: red;">Stopover:14H 05m</h5>
        </div>
    </div>
    <div class="row ml-15">
        <div class="col-md-8">
            <h5 class="list-title" style="color:#000000">BRU-BRUSSELS to LHR-HEATHROW</h5>
            <ul class="list" style="color:#000000">
                <li>SN BRUSSELS AIRLINES&nbsp;</li>
                <li>SN2103</li>
                <li>Depart 21:10 Arrive 21:20</li>
                <li>Duration:15H 15m</li>
            </ul>
        </div>
    </div>
</div>-->
    ...
    {
        $CardNumber=  $this->input->post('CardNumber');
        $res=$this->BaseModel->del('customer_receipt_details',array('id'=>$CardNumber));
        if($res>0)
        {
        echo $res;
        }
    ...
    {
        $dataArray= json_decode($this->input->post('dataArray'));
        $tableName=$dataArray->table;
        $primaryId=$dataArray->primaryKey;
        $fieldsIds=$this->input->post('fieldData');
//        print_r($dataArray);
        $referenceTable='';
        $referenceFields='';
        $isDeleteReference=false;
        if(array_key_exists('reference', $dataArray))
        {
            $isDeleteReference=true;
            $referenceTable=$dataArray->referenceTable;
            $referenceFields=$dataArray->foreignKey;
        }
        else
        {
            $referenceTable='';
...
//        exit();
        $count=count($fieldsIds);
      if($count >0)
       {
          $response=array();
          foreach($fieldsIds as $obj)
              {
                 $runTimeIdRow=$obj[0];
                $idDel=$obj[1];
                    if($isDeleteReference==true && !empty($referenceTable) && !empty($referenceFields))
                    {
                        $this->BaseModel->del($referenceTable,array($referenceFields=>$idDel));
                    }
                $restDel=$this->BaseModel->del($tableName,array($primaryId=>$idDel));
                if($restDel >0)
                   {
                        $response[]=$runTimeIdRow;
                   }
              }
              echo json_encode($response);
       }
