           ...
           foreach($dr_supplier as $key=> $drentry)
            {
               $dr_supplierSplit='';
               $cr_supplierSplit='';
               if($dr_supplier[$key]!=$cr_supplier[$key])
               {
           ...
           foreach($dr_supplier as $key=> $drentry)
            {
               ...
               if($dr_supplier[$key]!=$cr_supplier[$key])
               {
                ...
                if($dr_supplierSplit[0]=='Customer')
                 ...
                 {
                     if($dr_supplierSplit[0]=='Card Terminal-BH Squareup' || $dr_supplierSplit[0]=='Profit & Loss on Cancellations-UK' )
                     {
                         ...
                         $resp=$this->BaseModel->save('payment',array('transectionId'=>$transectionId,'pay_to'=>$dr_supplierSplit[0],'pay_by'=>$cr_supplierSplit[0],'amount'=>$dr_Amount[$key],'booking_ref'=>$dr_Reference[$key],'pay_date'=>$tdate,'pay_type'=>'Dr','description'=>$transection_description,'payment_nature'=>$dr_supplierSplit[1],'brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
//                         $resp3=$this->BaseModel->save('payment',array('transectionId'=>$newTransectionID,'pay_to'=>'Expense- BH Squareup Card Charges','pay_by'=>$cr_supplierSplit[0],'amount'=>$cardCharges,'booking_ref'=>$cr_Reference[$key],'pay_date'=>$tdate,'pay_type'=>'Cr','description'=>$transection_description,'payment_nature'=>'Card Charges','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
//                         $resp4=$this->BaseModel->save('payment',array('transectionId'=>$newTransectionID,'pay_to'=>$cr_supplierSplit[0],'pay_by'=>'Expense- BH Squareup Card Charges','amount'=>$cardCharges,'booking_ref'=>$dr_Reference[$key],'pay_date'=>$tdate,'pay_type'=>'Dr','description'=>$transection_description,'payment_nature'=>'expense','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
                ...
                {
                    if($cr_supplierSplit[0]=='Card Terminal-BH Squareup' || $cr_supplierSplit[0]=='Profit & Loss on Cancellations-UK')
                     {
                         $cardCharges=round(($cr_Amount[$key]*3)/100,2); 
                         $newTransectionID=$this->BaseModel->save('transection',array('booking_id'=>$cr_Reference[$key]));
                         $resp=$this->BaseModel->save('payment',array('transectionId'=>$transectionId,'pay_to'=>$cr_supplierSplit[0],'pay_by'=>$dr_supplierSplit[0],'amount'=>$cr_Amount[$key],'booking_ref'=>$cr_Reference[$key],'pay_date'=>$tdate,'pay_type'=>'Cr','description'=>$transection_description,'payment_nature'=>$cr_supplierSplit[1],'brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
//                         $resp3=$this->BaseModel->save('payment',array('transectionId'=>$newTransectionID,'pay_to'=>'Expense- BH Squareup Card Charges','pay_by'=>$cr_supplierSplit[0],'amount'=>$cardCharges,'booking_ref'=>$cr_Reference[$key],'pay_date'=>$tdate,'pay_type'=>'Cr','description'=>$transection_description,'payment_nature'=>'Card Charges','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
//                         $resp4=$this->BaseModel->save('payment',array('transectionId'=>$newTransectionID,'pay_to'=>$cr_supplierSplit[0],'pay_by'=>'Expense- BH Squareup Card Charges','amount'=>$cardCharges,'booking_ref'=>$dr_Reference[$key],'pay_date'=>$tdate,'pay_type'=>'Dr','description'=>$transection_description,'payment_nature'=>'expense','brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
                         $oldCardCharges=getCardCharges($cr_Reference[$key]);
                         $newChargees=$oldCardCharges+$cardCharges; 
           ...
           foreach($dr_supplier as $key=> $drentry)
            {
               ...
               if($dr_supplier[$key]!=$cr_supplier[$key]){
                ...
                if($dr_supplierSplit[0]=='Customer')
                 ...
                 {
                    if($dr_supplierSplit[0]=='Card Terminal-BH Squareup')
                    {
                         $cardCharges=round(($dr_Amount[$key]*3)/100,2); 
                         $newTransectionID=$this->BaseModel->save('transection',array('booking_id'=>$dr_Reference[$key]));
                         $resp=$this->BaseModel->save('payment',array('transectionId'=>$transectionId,'pay_to'=>$dr_supplierSplit[0],'pay_by'=>$cr_supplierSplit[0],'amount'=>$dr_Amount[$key],'booking_ref'=>$dr_Reference[$key],'pay_date'=>$tdate,'pay_type'=>'Dr','description'=>$transection_description,'payment_nature'=>$dr_supplierSplit[1],'brand'=>$brand,'login_flag'=>$loginFlag,'login_person'=>$loginId));
