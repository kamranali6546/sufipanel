 ...
 */
 public function __construct()
    ...
    public function index()
    {
        ...
        $data['records2']=$this->BaseModel->getWhereM('inquiry',$condition2);
    ...
    public function picked()
    {
        ...
        $date=date('Y-m-d');
         $endDate=date('Y-m-d',strtotime('-30 days',strtotime($date)));
        //$orderby="id desc";
        $group_by="picke_date";
        ...
        {
            $condition=" enqdate <> '0000-00-00' AND enqdate Between $endDate AND $date AND  status='2' ";
...
//        }
        ...
        {
            $qry=" Select * from inquiry where  status='2' AND enqdate <> '0000-00-00' AND  enqdate Between '$endDate' AND '$date' group by picke_date order by id Desc";
//            $record=$this->BaseModel->getWhereMOrder('inquiry',array('status'=>2),'id','desc',$group_by);
            $record=$this->BaseModel->getQuery($qry);
        }
...
//                   $res3=$mysqli->query($qry3);
 ...
 $output.='<div class="panel-body" style="display:none;">
    ...
    public function getCommentModel()
    {
        ...
        $commments='';
        $flightItener=  $obj->flight_itenery;
        $flightArray=unserialize(base64_decode($flightItener));
        $dataArray['itenery']=$flightItener;
...
//        print_r($flightArray);
       ...
       $style="<style>.list li {    text-decoration: none;list-style: none;}</style>";
        $modalHtml=$style.' <div class="modal-header">
					...
					<div class="col-md-12">
						...
						<table class="table" width="100%">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>'. date('d-M-y',  strtotime($obj->return_date)).'</td>
                                                    </tr>
                                                    <tr style="background: #ffeb3b75;">
         ...
         if($flag==1)
            {
                ...
                {
                    $resulCout=$this->BaseModel->getQuery('select inquiry.id as iid,inquiry.picked_by,inquiry.picke_date,inquiry.picked_time,inquiry.flight_from,inquiry.departure_date,inquiry.return_date,inquiry.name,inquiry.prefered_airline,inquiry.ticket_type,inquiry.ticket_class,inquiry.fare,inquiry.adult,inquiry.child,inquiry.infaunt,inquiry.status,inquiry.enquiry_date,inquiry.customer_email,inquiry.destination,inquiry.customer_instr,inquiry.customer_phone,admin.id as aid,admin.login_name from inquiry LEFT JOIN admin  ON  inquiry.picked_by=admin.id where inquiry.status="2" order by iid desc ');
                    $total_row=count($resulCout);
                    $config = array();
                    $config["base_url"] = base_url()."inquiry/followup/";
                    $config["total_rows"] = $total_row;
                    ...
                    //$page = ($this->uri->segment(3)) ? $this->uri->segment(3)-1*$config["per_page"] : 0;
                    $data['result']=$this->BaseModel->getQuery('select inquiry.id as iid,inquiry.picked_by,inquiry.reveicved_from,inquiry.device,inquiry.os,inquiry.brand_code,inquiry.inquiry_regin,inquiry.picke_date,inquiry.picked_time,inquiry.flight_from,inquiry.departure_date,inquiry.return_date,inquiry.name,inquiry.prefered_airline,inquiry.ticket_type,inquiry.ticket_class,inquiry.fare,inquiry.adult,inquiry.child,inquiry.infaunt,inquiry.status,inquiry.enquiry_date,inquiry.customer_email,inquiry.destination,inquiry.customer_instr,inquiry.customer_phone,admin.id as aid,admin.login_name from inquiry LEFT JOIN admin  ON  inquiry.picked_by=admin.id where inquiry.status="2" order by iid desc limit '.$config['per_page'].'  offset '.$page);
                }
            }
        else
            {
             if(!empty($startDate) && !empty($endDate) && !empty($agentId) && $agentId >0)
                ...
                {
                    $resulCout=$this->BaseModel->getQuery('select inquiry.id as iid,inquiry.picked_by,inquiry.picke_date,inquiry.picked_time,inquiry.flight_from,inquiry.departure_date,inquiry.return_date,inquiry.name,inquiry.prefered_airline,inquiry.ticket_type,inquiry.ticket_class,inquiry.fare,inquiry.adult,inquiry.child,inquiry.infaunt,inquiry.status,inquiry.enquiry_date,inquiry.customer_email,inquiry.destination,inquiry.customer_instr,inquiry.customer_phone,admin.id as aid,admin.login_name from inquiry LEFT JOIN admin  ON  inquiry.picked_by=admin.id where inquiry.picked_by='.$userId.' AND inquiry.status="2" order by iid desc ');
                    $total_row=count($resulCout);
                    $config = array();
                    $config["base_url"] = base_url()."inquiry/followup/";
                    $config["total_rows"] = $total_row;
                   ...
                   // $page = ($this->uri->segment(3)) ? $this->uri->segment(3)-1*$config["per_page"] : 0;
                   $data['result']=$this->BaseModel->getQuery('select inquiry.id as iid,inquiry.picked_by,inquiry.reveicved_from,inquiry.device,inquiry.os,inquiry.brand_code,inquiry.inquiry_regin,inquiry.picke_date,inquiry.picked_time,inquiry.flight_from,inquiry.departure_date,inquiry.return_date,inquiry.name,inquiry.prefered_airline,inquiry.ticket_type,inquiry.ticket_class,inquiry.fare,inquiry.adult,inquiry.child,inquiry.infaunt,inquiry.status,inquiry.enquiry_date,inquiry.customer_email,inquiry.destination,inquiry.customer_instr,inquiry.customer_phone,admin.id as aid,admin.login_name from inquiry LEFT JOIN admin  ON  inquiry.picked_by=admin.id where inquiry.picked_by='.$userId.' AND inquiry.status="2" order by iid desc limit '.$config['per_page'].'  offset '.$page); 
                }
            }
    ...
    public function otherDownloadSheet()
    {
        ...
        if(!empty($endDate))
         ...
         $this->excel->setActiveSheetIndex(0);
                ...
                $this->excel->getActiveSheet()->setCellValue('Q2', 'Inquiry Id');
                ...
                $condition1="admin.id";
                $select="name,picked_by,inquiry.id as inqId,customer_email,customer_phone,destination,flight_from,departure_date,return_date,adult,child,infaunt,prefered_airline,cost_price,sale_price,pnr,customer_instr,admin.login_name";
               ...
               // print_r($resultInquery);
                ...
                foreach ($resultInquery->result_array() as $row){  
                    $inquiryAgentId=$row['picked_by'];
                    $rowData=array();
                    $commentNote='';
                    $idInq=$row['inqId'];
                    $qry2=" Select * from  agent_comments where inquiry_id='$idInq' AND  agent_id='$inquiryAgentId'  Order by id Desc limit 1 ";
                    $commentData=commentGet($qry2);
                    //print_r($commentData);
                    if(!empty($commentData))
                    {
                       $commentNote=$commentData['remakts'];
                    }
                    else
                    {
                        $commentNote=$row['customer_instr'];
                    }
                    $rowData['name']=$row['name'];
                    $rowData['customer_email']=$row['customer_email'];
                    $rowData['customer_phone']=$row['customer_phone'];
                    $rowData['destination']=$row['destination'];
                    $rowData['flight_from']=$row['flight_from'];
                    $rowData['departure_date']=$row['departure_date'];
                    $rowData['return_date']=$row['return_date'];
                    $rowData['adult']=$row['adult'];
                    $rowData['child']=$row['child'];
                    $rowData['infaunt']=$row['infaunt'];
                    $rowData['prefered_airline']=$row['prefered_airline'];
                    $rowData['cost_price']=$row['cost_price'];
                    $rowData['sale_price']=$row['sale_price'];
                    $rowData['pnr']=$row['pnr'];
                    $rowData['customer_instr']=$commentNote;
                    $rowData['login_name']=$row['login_name'];
                    $rowData['inqId']=$row['inqId'];
                ...
                $exceldata[] = $rowData;
                    ...
                    $this->excel->getActiveSheet()->getStyle('A3:Q3')->getFill()->getStartColor()->setRGB('#fff000');
