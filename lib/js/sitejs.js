...
function inquiryRestore(inqryId)
{
    if(inqryId >0)
    {
         alertify.confirm("Are you sure to Restore the following Inquiry? Inquiry ID:"+inqryId+" ",
         function(ev)
         {
             showLoder();
             if(ev)
             {
                  $.ajax({
            type:'POST',
            url:ajaxUrl+'Inquery/RestoreBox',
            data:{ids:inqryId},
            cache:false,
            success:function(resp)
            {
...
function inquiryDelete2(inqryId)
{
   alertify.confirm("Are you sure to delete the following Inquiry? Inquiry ID:"+inqryId+" ",
  function(ev){
      if(ev){
...
//        $('#supplier_agentError').text('Please Fill the Supplier Agent Email!');
//        return false;
//    }
...
{
    alertify.confirm('Are You sure To Delete this Supplier ?',function(ev){
       if(ev)
       {
          $.ajax({
              type:'POST',
...
//            $('#SendReceipt .content2').unbind();
//            $('#SendReceipt .content2').richText();
//            $('#SendReceipt #bookingIdhidenSendReceipt').val(obj.bookingNum);
//            $('#SendReceipt #richText-moowvw').html(obj.detailsData);
...
//                    console.log(objData);
                    $('#TicketOrderEmail .contentTicketOrderEditor').richText();
                    $('#TicketOrderEmail #messageInn').val(objData.innterlMessage);
                    $('#TicketOrderEmail .richText-editor').html(objData.innterlMessage);
...
$("#ticketOderEmailSendForm").on('submit',(function(e) {
        ...
        var mshHtml=$('#richText-3xg4h').html();
        $('#messageInn22').val(mshHtml);
